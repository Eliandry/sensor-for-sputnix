...
uint8_t ds18b20_Reset(void)
{
  ...
  DelayMicro(485);//задержка на 480 микросекунд
  GPIOB->ODR |= GPIO_ODR_ODR11;//высокий уровень
  DelayMicro(65);//задержка на 60 микросекунд
  status = GPIOB->IDR & GPIO_IDR_IDR11;//проверяем уровень
  DelayMicro(500);//задержка  на 480 микросекунд
  return (status ? 1 : 0);// результат
}
//--------------------------------------------------
uint8_t ds18b20_ReadBit(void)
{
  ...
  GPIOB->ODR |= GPIO_ODR_ODR11;//высокий уровень
  DelayMicro(13);
  bit = (GPIOB->IDR & GPIO_IDR_IDR11 ? 1 : 0);// уровень
  DelayMicro(45);
  return bit;
}
//-----------------------------------------------
